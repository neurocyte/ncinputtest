...
pub const Context = struct {
    ...
    pub fn getvec_nblock(self: *const Self, ni: []Input) ![]Input {
        const ret = notcurses_getvec_nblock(self.nc, ni.ptr, @intCast(ni.len));
        return if (ret < 0)
            error.NCGetFailed
        else
            ni[0..@intCast(ret)];
    ...
    fn notcurses_getvec_nblock(n: ?*c.struct_notcurses, ni: [*c]c.ncinput, vcount: c_int) c_int {
        var ts: c.struct_timespec = c.struct_timespec{
            .tv_sec = @as(c.__time_t, @bitCast(@as(c_long, @as(c_int, 0)))),
            .tv_nsec = @as(c.__syscall_slong_t, @bitCast(@as(c_long, @as(c_int, 0)))),
        };
        return c.notcurses_getvec(n, &ts, ni, vcount);
    }
    ...
    pub fn cursor_enable(self: Self, y: usize, x: usize) !void {
        const err = c.notcurses_cursor_enable(self.nc, @intCast(y), @intCast(x));
    ...
    pub fn cursor_yx(self: Self) !struct { y: isize, x: isize } {
        ...
        return if (err != 0) error.NCCursorDisableFailed else .{ .y = y, .x = x };
    }
...
pub const Plane = struct {
    ...
    pub fn at_cursor_cell(self: *const Self, cell: *Cell) !usize {
    ...
    pub fn at_yx_cell(self: *const Self, y_: c_int, x_: c_int, cell: *Cell) !usize {
    ...
    /// 'ncp' is destroyed.
    pub fn base(self: *const Self, cell: *Cell) void {
    ...
    pub fn putstr(self: Self, gclustarr: [*c]const u8) !usize {
        const ret = c.ncplane_putstr(self.n, gclustarr);
    ...
    pub fn putstr_aligned(self: Self, y_: c_int, align_: Align, s: [*c]const u8) !usize {
        const ret = c.ncplane_putstr_aligned(self.n, y_, @intFromEnum(align_), s);
        return if (ret < 0) error.NCPlanePutStrFailed else @intCast(ret);
    }
    ...
    pub fn cell_load(self: *const Self, cell: *Cell, gcluster: [*c]const u8) !usize {
...
pub fn input() Input {
    return comptime if (std.meta.trait.hasField("eff_text")(Input)) .{
        .id = 0,
        .y = 0,
        .x = 0,
        .utf8 = [_]u8{0} ** 5,
...
pub fn ncstrwidth(egcs: [:0]const u8) !usize {
...
/// 16-bit wchar_t!
pub fn ncegc_len(egcs: [:0]const u8, colcount: *c_int) !usize {
    const ret = c.ncegc_len(egcs.ptr, colcount);
